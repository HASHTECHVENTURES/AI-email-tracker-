...
import { formatBytes, formatInr, formatUsd } from '@/lib/admin/format';
        ...
        <div className="space-y-8">
          <section>
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <StatCard label="Platform API" value={formatInr(billing.platform_totals.api_cost_inr)} sub={formatUsd(billing.platform_totals.api_cost_usd)} />
              <StatCard label="Platform storage" value={formatInr(billing.platform_totals.storage_cost_inr)} sub={formatBytes(billing.platform_totals.storage_bytes)} />
              <StatCard label="Total billable" value={formatInr(billing.platform_totals.total_cost_inr)} accent="text-brand-600" />
              <StatCard label="API calls" value={billing.platform_totals.api_calls.toLocaleString()} sub={`${billing.platform_totals.total_tokens.toLocaleString()} tokens`} />
            </div>
          </section>
              ...
              <table className="w-full min-w-[900px] text-left text-sm">
                <thead className="border-b border-slate-100 bg-slate-50/80 text-xs uppercase tracking-wide text-slate-400">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right">API (INR)</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {billing.companies.map((row) => (
                    <tr key={row.company_id} className="hover:bg-slate-50/60">
                      ...
                      <td className="px-4 py-3 text-right tabular-nums">{formatInr(row.api_cost_inr)}</td>
