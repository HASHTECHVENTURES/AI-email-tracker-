...
export function requirePublicSupabaseEnv(
  ...
  if (!url?.trim() || !key?.trim()) {
    throw new Error(
      'Missing Supabase settings. In admin/.env.local set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY (copy from Supabase → Project Settings → API), then restart `next dev`.',
    );
  }
...
export function formatAuthClientError(err: unknown): string {
  if (err instanceof Error) {
    ...
    if (m === 'Failed to fetch' || m.includes('NetworkError') || m.includes('Load failed')) {
      return 'Could not reach Supabase. Check NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in admin/.env.local (must match Supabase → Project Settings → API), restart `next dev`, and confirm you are online.';
    }
    return m;
  }
